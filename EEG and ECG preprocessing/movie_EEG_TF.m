...
%% integrated within 5 frequency bands
% delta(1-4Hz) theta(4-7Hz) alpha(7-12Hz) beta(12-30Hz) gamma(30-45Hz)
% delta 1-3
...
Ha_beta = sum(TFR_Ha.powspctrm(:,find(TFR_Ha.freq==12):find(TFR_Ha.freq==30),:),2);
Ha_gamma = sum(TFR_Ha.powspctrm(:,find(TFR_Ha.freq==30):find(TFR_Ha.freq==45),:),2);
...
Fe_beta = sum(TFR_Fe.powspctrm(:,find(TFR_Fe.freq==12):find(TFR_Fe.freq==30),:),2);
Fe_gamma = sum(TFR_Fe.powspctrm(:,find(TFR_Fe.freq==30):find(TFR_Fe.freq==45),:),2);
...
Ne_beta = sum(TFR_Ne.powspctrm(:,find(TFR_Ne.freq==12):find(TFR_Ne.freq==30),:),2);
Ne_gamma = sum(TFR_Ne.powspctrm(:,find(TFR_Ne.freq==30):find(TFR_Ne.freq==45),:),2);

% save in movie_EEG_TS
movie_EEG_TS.happy.delta = Ha_delta;
movie_EEG_TS.happy.theta = Ha_theta;
movie_EEG_TS.happy.alpha = Ha_alpha;
movie_EEG_TS.happy.beta  = Ha_beta;
movie_EEG_TS.happy.gamma = Ha_gamma;

movie_EEG_TS.fear.delta = Fe_delta;
movie_EEG_TS.fear.theta = Fe_theta;
movie_EEG_TS.fear.alpha = Fe_alpha;
movie_EEG_TS.fear.beta  = Fe_beta;
movie_EEG_TS.fear.gamma = Fe_gamma;

movie_EEG_TS.neutral.delta = Ne_delta;
movie_EEG_TS.neutral.theta = Ne_theta;
movie_EEG_TS.neutral.alpha = Ne_alpha;
movie_EEG_TS.neutral.beta  = Ne_beta;
movie_EEG_TS.neutral.gamma = Ne_gamma;
